 ...
 *
 * sharer.php is the legacy path and needs no configuration, so it stays as the
 * default. Facebook's Sharing Debugger reports its missing fb:app_id as an
 * error to correct.
 *
...
export default function ShareButtons({ title, url, className = '' }: ShareButtonsProps) {
  ...
  return (
    <div className={`flex items-center gap-2 flex-wrap ${className}`}>
      <span className="text-sm text-gray-600 dark:text-gray-400 font-medium ml-1">مشاركة:</span>

      {/* Native share (mobile) */}
      <button
        onClick={handleNativeShare}
        className="md:hidden flex items-center gap-1.5 px-3 py-2 bg-primary text-white text-sm rounded-lg hover:bg-primary/90 transition-colors"
        title="مشاركة"
      >
      ...
      <div className="hidden md:flex items-center gap-2">
        ...
        <button
          ...
          title="نسخ الرابط"
          className="flex items-center gap-1.5 px-3 py-2 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 text-sm rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors"
        >
          {copied ? (
          ...
          <span className="hidden lg:inline">{copied ? 'تم النسخ' : 'نسخ الرابط'}</span>
        </button>
      </div>
    </div>
  );
